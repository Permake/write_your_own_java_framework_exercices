...
public final class  InterceptorRegistry {
  // private final HashMap<Class<? extends Annotation>, List<AroundAdvice>>
  // annotationMap
  // = new HashMap<>();
  ...
  private Invocation computeInvocation(Method method) {
    return CACHE.computeIfAbsent(method, m -> getInvocation(findInterceptors(method)));
  };
  ...
  // annotationMap.getOrDefault(annotation.annotationType(),
  ...
  List<Interceptor> findInterceptors(Method method) {
    return Stream.of(Arrays.stream(method.getDeclaringClass().getAnnotations()),
        ...
        .flatMap(s -> s)
        .flatMap(interceptor -> interceptorMap.getOrDefault(interceptor.annotationType(), List.of()).stream()).toList();
  }
  ...
  static Invocation getInvocation(List<Interceptor> interceptorList) {
    ...
    for (var interceptor : interceptorList.reversed()) {
      var tmp = invocation;
      invocation = (instance, method, args) -> interceptor.intercept(instance, method, args, tmp);
    }
    return invocation;
  }
